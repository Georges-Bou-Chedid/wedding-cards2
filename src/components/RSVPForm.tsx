  ...
  return (
    ...
    >
      ...
      <p className="font-body text-sm italic text-muted-foreground mb-8">
        Please reply before: June 20, 2026
      </p>

      {status === "success" ? (
        <p className="font-heading text-lg text-primary">{message}</p>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {names.map((name, i) => (
            <div key={i} className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 border border-border rounded-sm bg-background font-body text-sm text-foreground focus:outline-none focus:ring-1 focus:ring-primary"
                required
              />
              {names.length > 1 && (
                <button
          ...
          <button
            type="button"
            onClick={addName}
            className="font-heading text-sm text-primary border border-primary px-4 py-1.5 rounded-sm hover:bg-primary hover:text-primary-foreground transition-colors duration-300 tracking-widest uppercase"
          >
            + Add Guest
          </button>
          <div>
            <button
              ...
              className="mt-4 w-full font-heading text-sm tracking-widest uppercase bg-primary text-primary-foreground py-3 rounded-sm hover:bg-toile-dark transition-colors duration-300 disabled:opacity-50"
            >
